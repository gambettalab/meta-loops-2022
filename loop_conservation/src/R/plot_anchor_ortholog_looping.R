...
stopifnot(orthodb$gene_id %in% unlist(strsplit(match$lap$anchor_nearest_gene_id, ", ")))
...
anchors <- match$lap[, list(
  anchor_TSS_proximal = anchor_TSS_proximal,
  anchor_distance_to_TSS = as.integer(strsplit(anchor_distance_to_TSS, ", ")[[1]]),
  gene_id = strsplit(anchor_nearest_gene_id, ", ")[[1]],
  gene_symbol = strsplit(anchor_nearest_gene_symbol, ", ")[[1]]
), by = c("loop_id", "anchor", "anchor_id")]

message("Considering the ", length(unique(anchors$anchor_id)), " anchors of ", length(unique(anchors$loop_id)), " loops")
dt <- unique(anchors[, c("anchor_id", "anchor_TSS_proximal")])
message("Anchor split: ", sum(dt$anchor_TSS_proximal == 1), " promoter (TSS-proximal), ", sum(dt$anchor_TSS_proximal == 0), " intergenic (TSS-distal)")
...
dt <- unique(anchors_matched[has_other_anchor == T, c("anchor_id", "anchor_TSS_proximal")])
message()
message(nrow(dt), " loop anchors have a matching anchor in the other species")
message("Split of matched loop anchors: ", sum(dt$anchor_TSS_proximal == 1), " promoter (TSS-proximal), ", sum(dt$anchor_TSS_proximal == 0), " intergenic (TSS-distal)")
...
dt <- unique(anchors_matched_orthologs[has_other_anchor & has_other_gene, c("anchor_id", "anchor_TSS_proximal")])
message()
message(nrow(dt), " anchors have a matching anchor, and have an ortholog of anchor-associated gene(s)")
message("Split of matched loop anchors that have an ortholog: ", sum(dt$anchor_TSS_proximal == 1), " promoter (TSS-proximal), ", sum(dt$anchor_TSS_proximal == 0), " intergenic (TSS-distal)")
...
anchors_matched_orthologs_closest <- anchors_matched_orthologs[, list(other_anchor_gene_distance = mymin(other_anchor_gene_distance)),
  by = c("anchor_id", "anchor_TSS_proximal", "anchor_distance_to_TSS", "gene_id", "gene_symbol", "has_other_anchor", "other_anchor_looping", "has_other_gene")]
...
# sort according to the natural order of anchor_id
up <- unique(anchors_matched_orthologs_closest$anchor_id)
ups <- paste0("A", seq_along(up))
stopifnot(setequal(up, ups))
anchors_matched_orthologs_closest[, anchor_id := factor(anchor_id, ups)]
setkey(anchors_matched_orthologs_closest, anchor_id)

dt <- unique(anchors_matched_orthologs_closest[other_anchor_gene_distance <= max_TSS_distance, c("anchor_id", "anchor_TSS_proximal")])
message()
message(nrow(dt), " loop anchors are matching an ortholog (matching anchor in the other species overlaps an ortholog)")
message("Split of ortholog-matching loop anchors: ", sum(dt$anchor_TSS_proximal == 1), " promoter (TSS-proximal), ", sum(dt$anchor_TSS_proximal == 0), " intergenic (TSS-distal)")
...
anchors_for_plot <- unique(anchors_matched_orthologs_closest[,
  c("anchor_id", "anchor_TSS_proximal", "anchor_distance_to_TSS", "gene_id", "gene_symbol", "has_other_anchor", "other_chrom", "other_start", "other_end", "other_anchor_looping", "has_other_gene", "other_anchor_gene_distance", "other_gene_id", "other_gene_name")])

message()
message("Diagnostic table for resolving multi-matching cases:")
anchor_ids_to_resolve <- anchors_for_plot[, list(.N), by = "anchor_id"][N > 1, ]$anchor_id
print(anchors_for_plot[anchor_id %in% anchor_ids_to_resolve, ])

# first, in D. mel. take the gene with the closest TSS to the anchor
anchors_for_plot <- anchors_for_plot[, list(
  anchor_distance_to_TSS = min(anchor_distance_to_TSS),
  gene_id = gene_id[which(anchor_distance_to_TSS == min(anchor_distance_to_TSS))],
  gene_symbol = gene_symbol[which(anchor_distance_to_TSS == min(anchor_distance_to_TSS))],
  other_anchor_gene_distance = other_anchor_gene_distance[which(anchor_distance_to_TSS == min(anchor_distance_to_TSS))],
  other_gene_id = other_gene_id[which(anchor_distance_to_TSS == min(anchor_distance_to_TSS))],
  other_gene_name = other_gene_name[which(anchor_distance_to_TSS == min(anchor_distance_to_TSS))]
), by = c("anchor_id", "anchor_TSS_proximal", "has_other_anchor", "other_chrom", "other_start", "other_end", "other_anchor_looping", "has_other_gene")]
...
), by = c("anchor_id", "anchor_TSS_proximal", "anchor_distance_to_TSS", "gene_id", "gene_symbol", "has_other_anchor", "has_other_gene")]
...
anchors_for_plot <- anchors_for_plot[, list(
  ...
  other_gene_id = paste(unique(other_gene_id), collapse = ", "),
  other_gene_name = paste(unique(other_gene_name), collapse = ", ")
), by = c("anchor_id", "anchor_TSS_proximal", "anchor_distance_to_TSS", "has_other_anchor", "other_chrom", "other_start", "other_end", "other_anchor_looping", "has_other_gene", "other_anchor_gene_distance")]

# save the anchor ortholog annotation table
write.table(anchors_for_plot, file = paste0("data/loops/loops_D_mel_to_D_vir_anchor_orthologs.tsv"),
  quote = F, sep = "\t", row.names = F, col.names = T)

# order by anchor-to-TSS distance in D. mel.
setkey(anchors_for_plot, anchor_distance_to_TSS)
anchors_for_plot[, anchor_id := factor(anchor_id, anchor_id)]
...
pdf("analysis/plots/loops_D_mel_to_D_vir_anchor_orthologs.pdf", width = 4, height = 14)

p1 <- ggplot(anchors_for_plot, aes(x = anchor_distance_to_TSS / 1e3, y = anchor_id)) +
  geom_segment(aes(xend = 0, yend = anchor_id), color = "#e41a1c", alpha = 0.3) +
...
dt <- anchors_for_plot[, c("anchor_id", "anchor_TSS_proximal", "has_other_anchor", "other_anchor_looping", "has_other_gene")]
...
dt <- melt(dt, id.vars = "anchor_id")

p2 <- ggplot(dt[value > 0, ], aes(x = variable, y = anchor_id)) +
...
p3 <- ggplot(anchors_for_plot, aes(x = pmin(other_anchor_gene_distance / 1e3, 30), y = anchor_id)) +
  geom_segment(aes(xend = 0, yend = anchor_id), color = "#e41a1c", alpha = 0.3) +
  geom_vline(xintercept = 0) +
...
pdf("analysis/plots/loops_D_mel_to_D_vir_anchor_ortholog_looping.pdf", width = 2.5, height = 3)

# dt <- anchors_for_plot[has_other_anchor == T & has_other_gene == T & anchor_TSS_proximal == 1, ]
dt <- anchors_for_plot[has_other_anchor == T & has_other_gene == T, ]
stopifnot(!anyDuplicated(dt$anchor_id))
p <- ggplot(dt, aes(other_anchor_gene_distance, other_anchor_looping, color = other_anchor_looping)) +
  labs(x = "Distance to the ortholog in D. vir.", y = NULL) +
  scale_x_continuous(trans = "log1p", labels = scales::comma, breaks = c(1, 1e2, 1e4, 1e6)) +
  scale_y_discrete(labels = c(
    `FALSE` = paste0("Candidate\nanchor\ndoes not\nloop (", sum(dt$other_anchor_looping == F), ")"),
    `TRUE` = paste0("Candidate\nanchor\nloops (", sum(dt$other_anchor_looping == T), ")")
    ), limits = c(TRUE, FALSE)) +
  geom_violin() +
  geom_boxplot(width = 0.2, outlier.shape = NA) +
  geom_jitter(position = position_jitter(seed = 4242), alpha = 0.3) +
  coord_flip() +
  scale_color_manual(values = c(`FALSE` = "#0092CE", `TRUE` = "#00447C")) +
...
message("Distance to the ortholog in D. vir. does not significantly differ between looping and non-looping candidate anchors: Wilcoxon p = ",
  wilcox.test(dt[other_anchor_looping == T, ]$other_anchor_gene_distance, dt[other_anchor_looping == F, ]$other_anchor_gene_distance)$p.value)
